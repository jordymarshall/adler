...
import { z } from "zod";
import { WORK_HIERARCHY } from "../shared/coaching-framework.ts";

export const researchReviewSchema = z
  .object({
    needsGrounding: z.boolean().default(false),
    issues: z
      .array(
        z
          .object({
  ...
  .strict();
export const researchReviewInstructions = `${WORK_HIERARCHY}
When requiresConfirmation is true, the entire change bundle remains pending regardless of requestedExecution. Reject replies claiming those changes have already been saved, updated or booked; ask for accurate proposed wording instead. A ready-to-review plan is not an applied plan.
For requestedExecution=apply, verify every substantive adjustment is explicitly requested or necessary to reconcile the requested fields. An edit request does not authorize unrelated coaching changes, new domain choices or invented workload; those belong in a proposed bundle. Exact requested edits do not need redundant confirmation merely because the saved plan has adaptive structure. For requestedExecution=propose, the reply must describe the bundle as a suggestion awaiting acceptance.
Reject an action that merely restates an intermediate outcome, even if its label differs from the milestone. Require an executable input and a criterion for the person's work. Check input quantities against the person's chosen work and capacity; a numeric quota alone does not make it justified. For an in-place edit, reconcile the requested field with its input measure, milestone links, timing, capacity and current learning. Preserve reported and booked history; a changed label is not a completed milestone.
