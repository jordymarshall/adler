...
export class Service {
  ...
  update(userId: string, state: unknown, revision: number, requestId: string) {
    return this.locked(userId, () => {
      const data = validateWorkspace(state);
  ...
  async chat(
    ...
    return this.locked(userId, async () => {
      ...
      const turn = {
        ...
        channel,
        workspace: { ...snapshot.data, messages: undefined },
