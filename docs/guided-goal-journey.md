...
The core landing composition comes from `v2-design`, retaining Adler’s DM Sans typography. Marketing eyebrows are removed throughout; the adaptation heading reads “Life moves. Your plan should, too.” The core story progresses through three visible chapters with a scroll-driven connecting line. The detailed seven-screen walkthrough is always visible below it; its final screen shows how saved context informs later coaching. Both the assembled hero and walkthrough heading read “Reach your goals with a plan that adapts to you.” All progress graphs reuse `ProgressChart`: solid steps for recorded results, dashed steps for dated checkpoints. Compact landing examples share the same illustrative data and geometry as the detailed chart. Illustrations never write account data. Motion reduction, readable contrast, mobile layout, and the shared outcome chart remain covered by browser checks.
