...
The landing uses Adler Warm typography and one four-part journey: find a direction, make time, check in, then review and adapt. The app preview shows one Start plan action followed by closed disclosures for reasoning, progress, and conversations. Start plan advances to the scheduling example. The phone is centered in the connections step, with optional messaging and calendar integrations around it; alternate check-in controls are disclosed on request. The weekly review demonstrates remembered preferences and an approved scheduling change. The separate seven-screen walkthrough and floating dashboard are removed.

The flow heading reads “Reach your goals with a plan that adapts to you.” The review keeps “Life moves. Your plan should, too.” Graphs reuse `ProgressChart`, fill their containers at their natural aspect ratio, and use legible labels at desktop and mobile sizes. Solid steps show recorded results; dashed steps show dated checkpoints. Schedule changes leave both recorded outcomes and agreed checkpoints intact. Illustrations never write account data. Browser checks cover the primary action, disclosures, phone check-ins, scrolling, graph sizing, reduced motion, and accessibility.
