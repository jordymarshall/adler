...
Goals show actual activity and outcome evidence separately. Calendar identifies Adler work and preserves the surrounding commitments. Insights accumulates observations and working insights alongside their implications for planning, distinguishing saved adjustments from proposals. When a working inference exists, the overview shows it with its tentative status; otherwise it shows the recorded observation or hypothesis. The selected behavioural mechanism is visible alongside the hypothesis, while detailed sources remain expandable. Each entry expands into a straight reasoning path with observation, hypothesis, test, result, inference and next question. Framework fit, predictions, limits and original sources remain available in disclosures. The overview uses saved records rather than inventing personality labels or treating a reported preference as a proven effect. Existing records remain readable and become eligible for review; no historical inference is silently strengthened.
