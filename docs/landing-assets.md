...
- `LandingHero.tsx` names the AI goal coach within an unframed page composition: two transparent abstract forms unfold, turn and settle toward the app in twelve held scroll poses. Angled green shapes sit behind the artwork; the vibrant lime background fades into the next section, and artwork may cross the boundary. Motion reverses with scroll, adds no extra section, waits for the pictures to load and decode, and stays on the first pose with reduced motion. Responsive WebPs are local in `public/media/hero`; [art direction and exact generation prompts](research/landing-hero-art-direction.md) record their provenance. `LandingEvidence.tsx` supplies the tour’s before/after plan comparison, reports and research disclosure. “Start on the web” is removed. The user requested restoration of the existing App Store badge; it is visibly labelled “Coming soon” and disabled while its real destination is pending. The old badge also had no download URL.
- `LandingJourney.tsx` uses eight parts and 19 scroll states: goal creation, today's actions, a barrier conversation, dated progress, an experiment, trial results, the accepted next plan and continuing in iMessage. Forward scroll advances, backward scroll rewinds, and stopping holds. No timer, playback control, in-phone click/input/focus, decorative app tab or screenshot expansion is present. These authored demonstrations make no account, coach, message or calendar writes. Lime, peach and green chapter gradients and large cropped geometric fans extend the hero’s art direction. Fan poses use the existing scroll phase, with no animation timer; reduced motion fixes the shapes. The same backgrounds appear behind the short-screen document flow.
...
- Removed from the active page: standalone problem/ribbon introduction, decorative bloom and backdrop, the Google Play badge and footer download controls, connection catalog, repeated bottom controls and mountain finale. Historical assets remain source material and are not loaded.
