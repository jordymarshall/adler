...
The active landing page uses V2 artwork and the locally hosted Adler Warm font. Its opening animates scattered goal, action, progress, and coach cards into one next-step preview. Phones, short viewports, and reduced-motion preferences show a static preview. One six-step walkthrough covers goal definition, the weekly plan, time, a check-in, progress with proposed checkpoints, and a revised plan informed by learning. A centered phone shows check-ins alongside messaging and calendar connections. All graphs reuse the app’s `ProgressChart`. The video assets below belong to the earlier landing page and are retained as source assets; the active landing does not load them.
