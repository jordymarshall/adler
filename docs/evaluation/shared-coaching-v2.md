...
- Production host verification and real external-account messaging/calendar checks. The user has confirmed `withadler.com`; its dedicated `adler7/adler` Vercel project and both domain configurations are now prepared and verified. The persistent backend and `ADLER_BACKEND_URL` remain unconfigured. Domain verification does not count as a running app. See [deployment requirements](../deployment.md).
