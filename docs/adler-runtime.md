...
Initial goal setup uses the same configured model as ongoing coaching. It asks about the intended result, verification, target date, known baseline, available time, and next action. It then returns a typed goal/program proposal. Nothing is created until the user approves it. Manual setup needs no AI key. Learning assessments currently use comparable scores out of 10; other goals use verified milestones.
