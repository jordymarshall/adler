...
Targets and expected dates are separate. Forecast snapshots record the method, inputs, source IDs, assumptions and pace scenarios. Supported methods are cumulative observed outcome pace and a cumulative outcome-per-reported-behavior scenario. The latter matches outcomes to comparable behavior amounts over the chosen feedback delay and applies current reported follow-through once.
