...
- Deployment: the user supplied `withadler.com`. Created and configured the `adler7/adler` Vercel project, verified both domain configurations, and configured `www` to redirect to the canonical domain. Production deployment still requires the persistent backend origin/hosting access; the Vercel project has no backend environment variable. No hosted app health or persistence check has passed yet. See [deployment setup](deployment.md).
