...
export function GoalProjection({ data, goal, today = dateInZone(data.timeZone), compact = false }: { data: Data; goal: Goal; today?: string; compact?: boolean }) {
  ...
  if (compact) return <section className="goal-outlook-compact" aria-label={`Goal outlook for ${goal.title}`}><small>GOAL OUTLOOK</small><strong>{projection ? projectionDate(projection.expectedDate) : "Finish not estimated yet"}</strong>{!projection && <span>{goal.measure ? `${goal.measure.label} tracked separately from your actions` : `${goal.milestones.filter(milestone => milestone.done).length} of ${goal.milestones.length} milestones complete`}</span>}{projection && <><span>At {evidence?.paceSource === "Observed input pace" ? "the reported" : "the planned"} input pace</span>{figure}<small>Conditional range · not a success probability</small></>}<button className="text-link" onClick={() => setInspecting(true)}>{projection ? "How this is estimated ↗" : "Why no estimate yet? ↗"}</button>{inspecting && <Modal title="Your goal outlook" onClose={() => setInspecting(false)}><GoalProjection data={data} goal={goal} today={today} /></Modal>}</section>;
