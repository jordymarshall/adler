...
export function GoalProjection({ data, goal, today = dateInZone(data.timeZone) }: { data: Data; goal: Goal; today?: string }) {
  ...
  const x = (date: string) => 48 + 700 * (Date.parse(date) - Date.parse(start)) / span;
  ...
  return <section className="goal-projection" aria-label={`Goal timeline for ${goal.title}`}>
    <div className="projection-heading"><div><span className="section-kicker">YOUR PATH TO THE GOAL</span><h2>{model.current === null ? "Starting point needed" : `${number(model.current)} / ${number(model.target)} ${goal.measure?.unit ?? goal.unit ?? "milestones"}`}</h2><small>{model.observations.length ? `Last reported ${formatDate(model.observations.at(-1)!.date)}` : "Starting baseline"}</small></div><div><span>Projected finish</span><strong>{projection ? projectionDate(projection.expectedDate) : model.status}</strong><small>{projection && `${projectionDate(projection.earliestDate)} – ${projectionDate(projection.latestDate)}`}</small></div></div>
    <figure className="outcome-timeline">
      <span className="projection-axis-label">Goal attained (%)</span>
      <svg viewBox="0 0 800 295" role="img" aria-label={`${goal.title}. Goal attainment from 0 to 100 percent over time. ${projection ? `Projected finish ${projectionDate(projection.expectedDate)}. Shaded area and error bars show conditional goal attainment scenarios, not action completion uncertainty.` : model.status}`}>
        {[0, 50, 100].map(value => <g key={value}><line x1="48" x2="748" y1={y(model.target * value / 100)} y2={y(model.target * value / 100)} className={value === 100 ? "outcome-target" : "outcome-grid"} /><text x="37" y={y(model.target * value / 100) + 4} textAnchor="end">{value}%</text></g>)}
        <text x="746" y="19" textAnchor="end">{number(model.target)} {goal.measure?.unit ?? goal.unit ?? "milestones"} · Your goal</text>
        {projection && <>
        ...
        {!projection && <text x="400" y="147" textAnchor="middle">{model.status}</text>}
        {[0, .25, .5, .75, 1].map(f => { const date = addDays(start, Math.round(span / 86400000 * f)); return <text key={f} x={x(date)} y="277" textAnchor={f === 0 ? "start" : f === 1 ? "end" : "middle"}>{formatDate(date, { month: "short", year: "numeric" })}</text>; })}
      </svg>
      <figcaption><span><i className="outcome-key actual" />Reported outcome</span><span><i className="outcome-key projected" />Projected outcome</span><span><i className="outcome-key range" />Goal attainment range</span><span>Time →</span></figcaption>
    </figure>
