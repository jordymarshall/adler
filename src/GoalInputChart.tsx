...
export function GoalInputChart({ series, compact = false, onSelect }: {
  ...
  return <figure className={`goal-input-chart ${compact ? "compact" : ""}`}>
    <figcaption><strong>{measure.label} <span>({measure.unit})</span></strong>{!compact && <span>{series.reported.toLocaleString()} reported / {series.planned.toLocaleString()} planned{series.unknown ? ` · ${series.unknown} unknown` : ""}<small>Before today · visible period</small></span>}</figcaption>
